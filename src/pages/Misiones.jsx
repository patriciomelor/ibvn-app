...
export default function Misiones() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm">Cargando centro de misiones...</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      {/* Cabecera */}
      <div>
        <h2 className="text-3xl font-bold font-display text-slate-900 dark:text-white tracking-tight">Ministerio de Misiones</h2>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Conoce los campos misioneros, apoya en oración y capacítate para ir.</p>
      </div>
      ...
      {activeTab === 'pueblo' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {pueblo ? (
            <div className="lg:col-span-2 glass rounded-3xl p-6 border border-slate-200 dark:border-slate-850 space-y-6 relative overflow-hidden">
              ...
              <div>
                ...
                <h3 className="text-2xl sm:text-3xl font-bold font-display text-slate-900 dark:text-white">{pueblo.nombre}</h3>
                <p className="text-slate-500 dark:text-slate-400 text-xs mt-1">Región: {pueblo.region}</p>
              </div>

              {/* Estadísticas Demográficas */}
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-white/80 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-850 text-center">
                  <span className="text-slate-500 text-[10px] uppercase font-bold tracking-wider block mb-1">Población</span>
                  <span className="text-slate-700 dark:text-slate-200 text-sm font-bold">{(pueblo.poblacion / 1000000).toFixed(1)} M</span>
                </div>
                <div className="bg-white/80 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-850 text-center">
                  <span className="text-slate-500 text-[10px] uppercase font-bold tracking-wider block mb-1">Religión</span>
                  <span className="text-slate-700 dark:text-slate-200 text-sm font-bold truncate block">{pueblo.religion || 'Etnorreligión'}</span>
                </div>
                <div className="bg-white/80 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-850 text-center">
              ...
              <div className="space-y-3">
                <h4 className="font-bold text-sm text-slate-600 dark:text-slate-300">¿Por qué orar por ellos?</h4>
            ...
            <div className="lg:col-span-2 glass rounded-3xl p-8 text-center border border-slate-200 dark:border-slate-850">
          ...
          {pueblo && (
            <div className="glass rounded-3xl p-6 border border-slate-200 dark:border-slate-850 flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                <h4 className="font-bold font-display text-slate-900 dark:text-white text-md flex items-center space-x-2">
                ...
                <div className="p-4 bg-indigo-950/20 border border-indigo-900/10 rounded-2xl text-center">
                  ...
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Hermanos comprometidos</span>
                </div>
              </div>
              ...
              {!user ? (
                <button
                  ...
                  className="w-full flex items-center justify-center space-x-2 font-medium py-3 rounded-xl bg-white dark:bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 text-indigo-400 border border-slate-200 dark:border-slate-800 transition-all active:scale-[0.98] text-xs font-display"
                >
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-center space-x-2 font-medium py-3 rounded-xl transition-all active:scale-[0.98] text-xs font-display ${
                    isPraying
                      ...
                      : 'bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white shadow-lg shadow-indigo-950/50'
                  }`}
                >
                  {actionLoading ? (
      ...
      {activeTab === 'misioneros' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 px-1">Reportes desde el Campo</h3>
            
            {updates.length === 0 ? (
              <p className="text-slate-500 text-xs italic">Aún no hay reportes publicados.</p>
            ) : (
              updates.map((upd) => (
                <div key={upd.id} className="glass rounded-3xl p-6 border border-slate-200 dark:border-slate-850 space-y-4">
                  <div className="flex justify-between items-start border-b border-slate-200 dark:border-slate-850 pb-3">
                    <div>
                      <h4 className="font-bold text-sm text-slate-700 dark:text-slate-200">{upd.titulo}</h4>
                  ...
                  <p className="text-slate-600 dark:text-slate-300 text-xs leading-relaxed text-justify whitespace-pre-line">
                    {upd.contenido}
                  </p>
                </div>
              ))
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 px-1">Fichas de Misioneros</h3>
            
            {misioneros.map((mis) => (
              <div key={mis.id} className="glass rounded-2xl p-5 border border-slate-200 dark:border-slate-850 space-y-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h5 className="font-bold text-xs text-slate-700 dark:text-slate-200 leading-tight">{mis.nombre}</h5>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Servicio en: {mis.pais_servicio}</span>
                  </div>
                </div>
                ...
                <div className="space-y-3.5 text-[11px] leading-relaxed">
                  <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-justify">{mis.historia}</p>
                  </div>
                  <div className="bg-slate-50/80 dark:bg-slate-900/40 p-2.5 rounded-lg border border-slate-200 dark:border-slate-850">
      ...
      {activeTab === 'submodulos' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            {testimonios.map((tst) => (
              <div key={tst.id} className="glass rounded-2xl p-5 border border-slate-200 dark:border-slate-850 space-y-3 text-xs leading-relaxed">
                <p className="text-slate-600 dark:text-slate-300 italic text-justify">"{tst.contenido}"</p>
          ...
          <div className="space-y-4">
            ...
            {cursos.map((crs) => (
              <div key={crs.id} className="glass rounded-2xl p-5 border border-slate-200 dark:border-slate-850 flex flex-col justify-between min-h-[160px]">
                <div className="space-y-1.5">
                  <h5 className="font-bold text-xs text-slate-700 dark:text-slate-200">{crs.titulo}</h5>
                  <p className="text-slate-500 dark:text-slate-400 text-[11px] leading-relaxed">{crs.descripcion}</p>
                </div>
                ...
                {crs.url_material && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-center space-x-1.5 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:bg-slate-800 text-indigo-400 border border-slate-200 dark:border-slate-800 py-1.5 rounded-xl text-[10.5px] font-semibold mt-4 transition-all"
                  >
          ...
          <div className="space-y-4">
            ...
            {viajes.map((vj) => (
              <div key={vj.id} className="glass rounded-2xl p-5 border border-slate-200 dark:border-slate-850 space-y-4">
                <div className="space-y-1.5">
                  ...
                  <h5 className="font-bold text-xs text-slate-700 dark:text-slate-200">{vj.destino}</h5>
                  <p className="text-slate-500 dark:text-slate-400 text-[11px] leading-relaxed text-justify">{vj.descripcion}</p>
                </div>

                <div className="border-t border-slate-200 dark:border-slate-850 pt-3 text-[10.5px]">
                  <span className="text-slate-500 font-bold block mb-1">Requisitos:</span>
                  <p className="text-slate-500 dark:text-slate-400 leading-normal">{vj.requisitos}</p>
                </div>
              </div>
            ))}
          </div>
